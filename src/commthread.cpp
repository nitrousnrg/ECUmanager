...
los datos deben salir por algún lado hacia qt4application. TODO: send 'U' when escaping */
...
void commThread::setPort(QString portAddress)		//needed before run()
{
	if(serial == 0)
	{
...
{
	if(hardwareTarget == "ePIC project")	//alpha version of pic16F877's firmware
	{
		read_PIC_data();
	}
...
void commThread::read_PIC_data()
{
	if(serial)
	{
		...
		if( available1 >= 9)
		{
			...
			if( logging == true )
				tempLogFile->write(buffer);//,available);	//sacar available?
		}
	}
	else
	{
