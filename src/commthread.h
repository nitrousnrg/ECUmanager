...
public:
	...
	void run();
	void setPort(QString portAddress);	//needed before run()
	bool burnFlash(QString fileName);
	void setTarget(QString target){hardwareTarget = target;}	//needed before run() too
	int getChannel(int channel);					//puede acceder a todos los datos recibidos desde aqui
