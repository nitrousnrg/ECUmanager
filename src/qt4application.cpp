	...
	if( plot1 != NULL )			 //Viewer was opened
	{
		for(int i=0; i<=2; ++i)
		{
			switch(plot1->getChannel(i))
			{
				...
				case 1:
								 //RPM
					plot->desplazar(serialThread->getFChannel(commThread::RPM)/50);
					break;
				case 2:
...
void qt4application::disconnectECU()
{
	...
	delete serialThread;
								 ...
								 //Cargo los cambios de la tabla a la VE (si son correctos)
void qt4application::checkChange(QTableWidgetItem *item)
{
	int row = item->row();
...
void qt4application::openECUslot()
{
	...
	if(available > 900)
	{
		...
		for(i=i; dataBurst[i]<=251; i+=3)
		{
			if(dataBurst[i] == (dataBurst[i+3] - 1))
			{
				...
				IgnTable[11-dataBurst[i]/21][dataBurst[i]%21].angle = dataBurst[i+1]*6;
								 //Fix: round to 2 digits after comma
				IgnTable[11-dataBurst[i]/21][dataBurst[i]%21].angle += (float)dataBurst[i+2]*6/255;
			}
			else
				i -= 2;
		}
		...
		for(int j=0;j<12;++j,i+=2)
		{
			...
			fuelAdv[j].angle = fuelAdv[j].byte[1] * 6;
								 //Fix: round to 2 digits after comma
			fuelAdv[j].angle += (float)fuelAdv[j].byte[0] * 6 / 255;
	...
	//openECUtimer->~openECUtimer();
								 ...
								 //programar!
void qt4application::sendFilesInit()
{
...
bool qt4application::saveFile(const QString &fileName)
{
	...
	file.write("Displacement (cm\302\263):\n");
	...
	file.write("Injector Flow (cm\302\263/min):\n");
	...
	file.write("\n\nTemperature correction table (\302\260C vs enrich %)\n");
...
{
	//aqu guarda toda la nueva configuracin (de todos los dilogos posibles)
