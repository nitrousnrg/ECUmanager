...
void Label::update()
{
	...
	QFontInfo info(font_);
								 ...
								 //fm.boundingRect(text_)  misbehaviour under linux;
	...
	{
								 // draw empty space else //todo
		r =     QRect(QPoint(0,0),fm.size(Qwt3D::SingleLine, QString(" ")));
		#if QT_VERSION < 0x040000
		r.moveBy(0, -r.top());
		#else
		r.translate(0, -r.top());
	...
	#endif
								 // flipped 32bit RGBA ?
	tex_ = QGLWidget::convertToGLFormat( buf_ );
}
